...
echo "后台启动前端..."
nohup npm run client >"$WEB_LOG_FILE" 2>&1 &
web_bg_pid="$!"
...
echo "启动成功"
echo "前端: http://$HOST:$WEB_PORT (PID: $web_listening_pid)"
echo "后端: http://$HOST:$API_PORT (PID: $api_listening_pid)"
